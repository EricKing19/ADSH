...
class DAGHLoss(nn.Module):
    ...
    def forward(self, F_batch, B_batch):
        ...
        var_loss = self.lambda_2 * ((F_batch - mean_F) ** 2)

        # loss = 0.5 * (reg_loss.sum ()  - var_loss.sum ()) / (batch_size * self.code_length) + 0.5 * oth_loss / (batch_size * self.code_length)
        loss = 0.5 * (reg_loss.sum () + bla_loss.sum() - var_loss.sum ()) / (batch_size * self.code_length) + 0.25 * oth_loss / (batch_size * self.code_length)
        return loss
