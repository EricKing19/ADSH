...
import utils.DAGH_no_Ink_tr_loss as dl
...
def _dataset(dataname):
    ...
    if dataname=='CIFAR10':
        testlabels_ = load_label('train_label.txt', rootpath)
...
if __name__=="__main__":
    ...
    logdir = '-'.join(['log/test',opt.dataname, datetime.now().strftime("%y-%m-%d-%H-%M-%S")])
    _logging()
    _record()
