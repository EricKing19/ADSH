...
import utils.DC_dataset_flip as dataset
...
parser.add_argument('--dataname', default='COCO', type=str,
                    help='MirFlickr, NUSWIDE, COCO, MNIST, CIFAR10')
...
parser.add_argument('--lambda-1', default=0.001, type=int,
...
def _dataset(dataname):
    ...
    if dataname=='NUSWIDE':
        ...
        dset_database = dataset.CIFAR_10 ('database_img.txt', 'database_label.txt', transformations)
        dset_test = dataset.CIFAR_10 ('test_img.txt', 'test_label.txt', transformations)
    elif dataname == 'MNIST':
    ...
    def DC_load_label_CIFAR10(filename, DATA_DIR):
    ...
    if dataname == 'MNIST':
        databaselabels = DC_load_label_MNIST('training.pt', root = '/home/dacheng/PycharmProjects/ADSH_pytorch/data/processed/')
        testlabels = DC_load_label_MNIST('test.pt', root = '/home/dacheng/PycharmProjects/ADSH_pytorch/data/processed/')
    elif dataname == 'CIFAR10':
        databaselabels = DC_load_label_CIFAR10('database_label.txt', 'data/CIFAR-10')
        testlabels = DC_load_label_CIFAR10('test_label.txt', 'data/CIFAR-10')
    else:
...
def calc_loss(B, F, Z, inv_A, lambda_1, lambda_2, lambda_3, code_length):
    ...
    print ('oth_loss:' + str (lambda_1 * oth_loss.sum ()/ num_train))
...
def DAGH_algo(code_length, dataname):
    ...
    for iter in range(max_iter):
        ...
        if iter == 0:
            ...
            for iteration, (train_input, train_input2, train_label, batch_ind) in enumerate (trainloader):
        ...
        for epoch in range(epochs):
            for iteration, (train_input, train_input2, train_label, batch_ind) in enumerate(trainloader):
                ...
                batch_grad = get_batch_gard(B, left, Z_T, batch_all)/(1*batch_size)
                ...
                B_cuda = Variable (torch.from_numpy (B[:,batch_all]).type (torch.FloatTensor).cuda ())
                # optimizer.zero_grad ()
...
if __name__=="__main__":
    ...
    logdir = '-'.join(['log/Flip',opt.dataname, datetime.now().strftime("%y-%m-%d-%H-%M-%S")])
    _logging()
    _record()
