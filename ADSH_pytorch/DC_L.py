...
parser.add_argument('--lambda-1', default='0', type=str,
    ...
    for iter in range(max_iter):
        ...
        for epoch in range(epochs):
            for iteration, (train_input, train_label, batch_ind) in enumerate(trainloader):
                ...
                output[1].backward (lambda_ba * batch_grad)
...
if __name__=="__main__":
    ...
    logdir = '-'.join(['log/ba',opt.dataname, datetime.now().strftime("%y-%m-%d-%H-%M-%S")])
    _logging()
    _record()
