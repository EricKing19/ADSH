...
import utils.cnn_model_DAGH_tanh as cnn_model
...
parser.add_argument('--dataname', default='SUN20', type=str,
                    help='MirFlickr, NUSWIDE, COCO, CIFAR10, CIFAR100, Mnist, fashion_mnist, STL10, SUN20, imageNet50')
...
parser.add_argument('--lambda-1', default='0.001', type=str,
                    help='hyper-parameter: oth-lambda-1 (default: 0.001,0.01,0.1,1,10,100)')
parser.add_argument('--lambda-2', default='0.001', type=str,
...
parser.add_argument('--gamma', default=0.001, type=int,
...
def B4_step(B, Z, inv_A, lambda_1, lambda_2, rho1, rho2, gamma):
    """
    Update B : F^t * W
    ...
    for iter_B in range(50):
        ...
        loss = calc_all_loss (B, Z, inv_A, Z1, Z2, Y1, Y2, rho1, rho2, lambda_1, lambda_2)
...
def calc_all_loss(B, Z, inv_A, Z1, Z2, Y1, Y2, rho1, rho2, lambda_1, lambda_2):
    """
    ...
    temp = np.dot (B, np.dot (Z, inv_A)) # k X m
    temp2 = np.dot (temp, Z_T) # k X n
    BAF = np.dot (temp2, B.transpose()) # k X k
    Tr_BLF = np.trace(np.dot(B, B.transpose()) - BAF)
    ...
    loss = Tr_BLF + oth_loss.sum () + bla_loss.sum () + z1_loss + z2_loss + y1_loss + y2_loss
...
def calc_loss(B, F, Z, inv_A, lambda_1, lambda_2, lambda_3, code_length):
    ...
    oth_loss = lambda_1 * ((np.dot(B, B.transpose ()) / num_train - nI_K) ** 2) / 4
    bla_loss = lambda_2 * (B.sum(1) **2) / 2
    ...
    loss = Tr_BLF + 0.5 * reg_loss.sum () + bla_loss.sum () + oth_loss.sum () + lambda_3 * L1_loss.sum()

    print ('Tr_BLF:'+ str(Tr_BLF / code_length / num_train))
    print ('reg_loss:' + str (reg_loss.sum () / code_length / num_train))
    print ('oth_loss:' + str (oth_loss.sum ()/ code_length))
    print ('bla_loss:' + str (bla_loss.sum ()/ code_length / num_train))
...
def adjusting_learning_rate(optimizer, learning_rate, flag = True):
    if flag:
...
if __name__=="__main__":
    ...
    logdir = '-'.join(['log/SADH',opt.dataname, datetime.now().strftime("%y-%m-%d-%H-%M-%S")])
    _logging()
    _record()
