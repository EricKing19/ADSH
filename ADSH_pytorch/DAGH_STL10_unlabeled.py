...
def DAGH_algo(code_length, dataname):
    ...
    '''
    nums, dsets, labels = load_dataset(dataname, unlabeled=True)
    ...
    '''
    model.eval()
    nums, dsets, labels = load_dataset(dataname, unlabeled=False)
    num_database, num_test = nums
    dset_database, dset_test = dsets
    database_labels, test_labels = labels
...
if __name__=="__main__":
    ...
    logdir = '-'.join(['log/noInk_un',opt.dataname, datetime.now().strftime("%y-%m-%d-%H-%M-%S")])
    _logging()
    _record()
