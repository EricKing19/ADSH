...
do
  ...
  do
    for lambda_3 in ${lambda_3s[*]}
      do
      python DAGH_meanF_Bup_epoch2.py --lambda-1 $lambda_1 --lambda-2 $lambda_2 --lambda-3 $lambda_3 --epochs=2 --max-iter=6 --learning-rate=0.01 --gpu 3
      echo '###################################'
