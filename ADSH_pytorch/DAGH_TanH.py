...
import utils.tanH_nn as cnn_model
...
def DAGH_algo(code_length):
    ...
    for iter in range(max_iter):
        ...
        if iter == 0:
            ...
            model = cnn_model.tanH_CNNNet (model, opt.arch, 10*(iter + 1) )
...
if __name__=="__main__":
    ...
    logdir = '-'.join(['log/log-DAGH-TanH-cifar10', datetime.now().strftime("%y-%m-%d-%H-%M-%S")])
    _logging()
    _record()
